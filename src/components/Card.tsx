    ...
    return (
        <>
            <div className={"card " + cardWidthType + " " + (uncontained ? "uncontainedCard" : "")}>
                <div className="cardTitleContainer"><a href={titleLink} target="_blank" rel="noreferrer" className="cardTitleLink">{((typeof title === "undefined" || title === "") ? <></> : <><h1 className="cardTitle">{title}</h1><span className="cardTitleIcons">{icons}</span></>)}</a>{cardOutButton}</div>
